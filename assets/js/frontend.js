...
document.querySelector('button#validate').addEventListener('click', () => {
  if (document.querySelector('#code').value == "") {
    ...
    const code = document.querySelector('#code').value
    document.querySelector('#exit').innerHTML = `<p>Texto descriptografado: </p><mark>${decrypt(termEncrypted, code)}</mark>`
  }
})
...
document.querySelector('#field').addEventListener("keydown", function (event) {
  if (event.key === "Enter") {
    event.preventDefault();
    document.querySelector('button#generate').click()
  }
});
document.querySelector('#field_exit').addEventListener("keydown", function (event) {
  if (event.key === "Enter") {
    event.preventDefault();
